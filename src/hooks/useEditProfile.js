...
import { getDownloadURL, ref, uploadString } from 'firebase/storage';
import { firestore, storage } from '../firebase/firebase';
import { doc, updateDoc, collection, query, where, getDocs } from 'firebase/firestore'; // Import necessary Firestore functions
import useUserProfileStore from '../store/userProfileStore';
...
const useEditProfile = () => {
  ...
  const isUsernameTaken = async username => {
    ...
    const q = query(usersRef, where('username', '==', username)); // Query to check if username exists
  ...
  const editProfile = async (inputs, selectedFile) => {
    ...
    try {
      // Check if the new username is taken, but only if the user is attempting to change it
      if (inputs.username && inputs.username !== authUser.username) {
        const usernameTaken = await isUsernameTaken(inputs.username);
        if (usernameTaken) {
          showToast('Error', 'Username already taken', 'error'); // Show error if username is taken
          setIsUpdating(false);
          return;
        }
      }
